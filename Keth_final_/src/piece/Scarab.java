...
public class Scarab extends Piece{
	...
	public Scarab(GamePanel panel, int color, int col, int row, int orientation) {
	...
	public void reactLaser(int dx, int dy) {
		...
		if (hasFirstReflection) {
			getPanel().shootLaser(getCol(), getRow(), dy, -dx, color == GamePanel.WHITE ? Color.WHITE : Color.RED);
		}
