...
public class GamePanel extends JPanel implements Runnable{
	...
	public void setPieces() {
		...
		pieces.add(new Obelisk (WHITE, 4, 8, 0));
		pieces.add(new Obelisk (WHITE, 6, 8, 0));
		...
		pieces.add(new Obelisk (RED, 5, 1, 0));
		pieces.add(new Obelisk (RED, 7, 1, 0));
...
private boolean laserFired = false; // Indique si le laser a été tiré dans ce tour
	...
	private void update() {
		...
		if (!mouse.pressed) {
			if (activeP != null) {
				if (validSquare) {
					...
					// Tirer le laser après que toutes les pièces aient été déplacées
					if (!laserFired) {
						if (currentColor == WHITE) {
							shootLaser(GamePanel.WHITE);
						} else {
							shootLaser(GamePanel.RED);
						}
						laserFired = true;
						changePlayer();
					}
		...
		// Réinitialise l'état pour permettre le tir au prochain tour
		if (mouse.pressed) {
			laserFired = false;
		}
	}
